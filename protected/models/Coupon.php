...
class Coupon extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('coupon_code, amount, start_date, end_date, coupon_description, status, created_date', 'required'),
			array('amount, status', 'numerical', 'integerOnly'=>true),
			array('coupon_code', 'length', 'max'=>100),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, coupon_code, amount, start_date, end_date, coupon_description, status, created_date', 'safe', 'on'=>'search'),
		);
	}
