...
$strWhereQuery = "WHERE user='$BlogUserName' ";
...
if(isset($ProceedFilter))
{
	...
	if($stat_day!="All"&&$stat_month!="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '".$stat_month." ".$stat_day.", ".$stat_year."%'";
	}
	else
	if($stat_day!="All"&&$stat_month=="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '% ".$stat_day.",%'";
	}
	else
	if($stat_day=="All"&&$stat_month!="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '".$stat_month."%'";
	}
	else
	if($stat_day=="All"&&$stat_month=="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '%".$stat_year."%'";
	}
	else
	if($stat_day!="All"&&$stat_month!="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '".$stat_month." ".$stat_date."%'";
	}
	else
	if($stat_day!="All"&&$stat_month=="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '%".$stat_dat.", ".$stat_year."%'";
	}
	else
	if($stat_day=="All"&&$stat_month!="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$BlogUserName' AND date like '".$stat_mont."%".$stat_year."%'";
	}
	...
	{
		$iTotal=SQLCount("user_statistics",$strWhereQuery);
		$iUniques = SQLCount("user_statistics","WHERE user='$BlogUserName' AND referer NOT LIKE '%".$strServerName."%' ");
		$iReloads = $iTotal - $iUniques;
	}
}
else
		...
		<table bgcolor="#EFEFFF" border="0"   width=500  style="border-width:1px 1px 1px 1px;border-style:solid;border-color:black" cellpadding="0" cellspacing="0">
		  	<tr>
				...
				if($chart_type=="days")
				{
					...
					$tableStat = DataTable("user_statistics","WHERE user='$BlogUserName' ORDER BY id DESC");	
					...
					while($arrStat = mysql_fetch_array($tableStat))
					{
						...
						if(array_key_exists($arrInfo[0], $arrStatValues))
						{
							...
							if(isset($check_all))
							{
								...
								$strRef = "";
								$strRef2 = $BLOG_DOMAIN . "/blog.php?user=".$BlogUserName;
								
								if($BLOG_URL_FORMAT == 1)
								{
									$strRef = $BlogUserName .".". $BLOG_DOMAIN;
								}
								else
								{
									$strRef = $BLOG_DOMAIN ."/".$BlogUserName;
								}
				...
				if($chart_type=="months")
				{
					...
					$tableStat = DataTable("user_statistics","WHERE user='$BlogUserName' ");	
...
RenderTable(
						...
						750,
						"WHERE user='".$BlogUserName."' AND trim(referer) <> ''  AND referer NOT LIKE '%".$BLOG_DOMAIN."%' ORDER BY id DESC",
						"",
						"",
						"index.php?category=home&action=statistics"
						);
