...
function Category_generateNavigationMenu($oLinkTexts,$oLinkActions){
	global $category,$action,$folder,$AuthGroup,$arrPermissions,$evLinkDescriptions;
	...
	$strOutput.="<table cellspacing=0 CELLPADDING=0>";
	$strOutput.="<tr>";
	for($i=0;$i<count($oLinkTexts);$i++){
		...
		else
		{
						...
						$strOutput.="<td onmouseover=\"document.getElementById('img".$i."').style.visibility='visible'\" onmouseout=\"document.getElementById('img".$i."').style.visibility='hidden'\">";
		}
		
		if(!strstr($oLinkTexts[$i],"add-on"))
	{
		$strOutput.="<A HREF='index.php?user=".$_SESSION['username']."&category=".$category."&action=".$oLinkActions[$i]."' style=\"font-size:11px;text-decoration:none\" > <b>".strtoupper(str_replace("[add-on]","",$oLinkTexts[$i]))."</b></font></A>";
	}
	else
	{
		$strOutput.="<A HREF='index.php?user=".$_SESSION['username']."category=".$oLinkActions[$i]."'  > ".str_replace("[add-on]","",$oLinkTexts[$i])."</font></A>";
	}
...
if($category!="home")
{
	...
	echo "<hr size=\"1\" width=\"100%\" color=\"#f7f7f7\"><br>";
}
?>	 
<?php
