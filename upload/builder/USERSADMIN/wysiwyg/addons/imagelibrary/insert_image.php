...
if($allowuploads && $_FILES['file']) {
	...
	if($upload) 
	{
		...
		move_uploaded_file($_FILES['file']['tmp_name'], $leadon .$AuthUserName."/". $ir .".".$ext);
		
		SQLInsert
		(
			"image",
			array("image_id","image_type","image_size","image_name","user"),
			array($ir,$ext,filesize($leadon . $_FILES['file']['name']),$_FILES['file']['name'],$AuthUserName)
		);
...
function insertImage() {
	...
	//WYSIWYG.insertImage("http://www.novavizia.com/wp-content/uploads/google-logo-big.jpg", width, height, align, border, alt, hspace, vspace, n);
  	window.close();
}
...
<td style="vertical-align: top;width: 180px; padding-left: 5px;">
	...
	<iframe id="chooser" frameborder="0" style="height:165px;width: 220px;border: 2px solid #FFFFFF; padding: 5px;" src="select_image.php"; ?>"></iframe>
</td>
</tr>
<tr>
