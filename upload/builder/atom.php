...
if(substr($user, 0, 5) == "notes")
{
				...
				echo "<link href=\"http://www.".$BLOG_DOMAIN."/\"/>\n";
				echo "<updated>".date($ATOM_DATE_FORMAT, time())."</updated>";
				echo "<author> 
    						<name>".$BLOG_DOMAIN."</name>
  						</author>";
  				echo "<id>http://www.".$BLOG_DOMAIN."</id>";
				...
				while($arrNote = mysql_fetch_array($tableNotes))
				{
					...
					echo "<link href=\"".htmlspecialchars(CreateLink3($arrNote["user"],'note/'.$arrNote["id"]))."\"/>\n";
					echo "<id>".htmlspecialchars(CreateLink3($arrNote["user"],'note/'.$arrNote["id"]))."</id>";		
					echo "<updated>".date($ATOM_DATE_FORMAT, $arrNote["date"] )."</updated>";					
					echo "<summary>".to_decimal(htmlentities(stripslashes(substr($arrNote["html"],0,200))))."".(strlen(strip_tags($arrNote["html"]))>=200?"...":"")."</summary>\n";
...
if(substr($user, 0, 5) == "blogs")
{
				...
				while($arrLastBlog = mysql_fetch_array($tableLastBlogs))
				{
					...
					echo "<link href=\"http://".BlogUrl($arrLastBlog["username"])."\"/>\n";
					echo "<id>http://".BlogUrl($arrLastBlog["username"])."</id>";		
					echo "<updated>".date($ATOM_DATE_FORMAT, $arrLastBlog['last_update'] )."</updated>";					
...
else
{
				...
				$arrBlog = DataTable("admin_users","WHERE username='".$user."'");
				
				echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>\n";
				echo "<feed xmlns=\"http://www.w3.org/2005/Atom\" >\n";
				
				
				echo "<title>Blog posts for ".$user."</title>\n";
				echo "<link href=\"http://www.".$BLOG_DOMAIN."\"/>\n";
				echo "<updated>".date($ATOM_DATE_FORMAT, $arrBlog["last_update"] )."</updated>\n";
				echo "<author>\n 
    						<name>".$user."</name>\n
  						</author>\n";
  				echo "<id>http://".BlogUrl($user)."/</id>\n";
							
								
										
					$bFirstNote = true;
					
					$arrNotes = DataTable("notes","WHERE user='".$user."' ORDER BY id DESC");
					...
					while($arrNote = mysql_fetch_array($arrNotes))
					{
						if(trim($arrNote["title"]) == "")
						{
							continue;
									...
									echo "<entry>\n";
									echo "<title>".to_decimal(htmlentities(stripslashes($arrNote["title"])))."</title>\n";
									echo "<link href=\"http://www.".$BLOG_DOMAIN."/blog.php?user=".$user."&amp;note=".$arrNote["id"]."\"/>\n";
									echo "<id>http://www.".$BLOG_DOMAIN."/blog.php?user=".$user."&amp;note=".$arrNote["id"]."</id>\n";		
									echo "<updated>".date($ATOM_DATE_FORMAT, $arrNote["date"] )."</updated>\n";					
									
									$arrLines = explode(".",strip_tags($arrNote["html"]));
									
									echo "<summary>".to_decimal(htmlentities(stripslashes($arrLines[0])))."</summary>\n";
