...
<table summary="" border="0" width="100%">
	<tr>
		<td width="39"><img src="images/icons/chart_bar.png" width="48" height="48" alt="" border="0"></td>
		<td class="blog_admin_header"><?php echo $STATISTICS_YOUR_BLOG;?></td>
	</tr>
</table>
<br>
...
$strWhereQuery = "WHERE user='$AuthUserName' ";
...
if(isset($ProceedFilter))
{
	...
	if($stat_day!="All"&&$stat_month!="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '".$stat_month." ".$stat_day.", ".$stat_year."%'";
	}
	else
	if($stat_day!="All"&&$stat_month=="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '% ".$stat_day.",%'";
	}
	else
	if($stat_day=="All"&&$stat_month!="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '".$stat_month."%'";
	}
	else
	if($stat_day=="All"&&$stat_month=="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '%".$stat_year."%'";
	}
	else
	if($stat_day!="All"&&$stat_month!="All"&&$stat_year=="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '".$stat_month." ".$stat_date."%'";
	}
	else
	if($stat_day!="All"&&$stat_month=="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '%".$stat_dat.", ".$stat_year."%'";
	}
	else
	if($stat_day=="All"&&$stat_month!="All"&&$stat_year!="All")
	{
		$strWhereQuery = "WHERE user='$AuthUserName' AND date like '".$stat_mont."%".$stat_year."%'";
	}
		...
		if(trim($strWhereQuery)!="")
	{
		...
		$iTotal=SQLCount("user_statistics",$strWhereQuery);
		$iUniques = SQLCount("user_statistics",$strWhereQuery." AND referer NOT LIKE '%".$strServerName."%' ");
		$iReloads = $iTotal - $iUniques;
	}
	else
	{
		$iTotal=SQLCount("user_statistics",$strWhereQuery);
		$iUniques = SQLCount("user_statistics","WHERE user='$AuthUserName' AND referer NOT LIKE '%".$strServerName."%' ");
		$iReloads = $iTotal - $iUniques;
	}
}
else
	...
	<tr>
		...
		<table summary="" border="0" width=100%>
	  	<tr>
			...
			{
				document.write("<td class=basictext width=0 style='display:none' >");
			}
			</script>
	  ...
	  <br><br>
		...
		<input type=radio name=chart_type value="days" <?php  if(isset($chart_type)&&$chart_type=="days") echo "checked";?>>
		...
		<?php echo $PER_MONTHS;?>
		&nbsp;&nbsp;
		<input type=checkbox name=check_all <?php if(isset($check_all)) echo "checked";?>><?php echo $SHOW_UNIQUE;?>
		&nbsp;&nbsp;
		<input type=submit value=" <?php echo $DISPLAY;?> " class=adminButton>
		</form>
		...
		<table bgcolor="#EFEFFF" border="0"   width=500  style="border-width:1px 1px 1px 1px;border-style:solid;border-color:black" cellpadding="0" cellspacing="0">
		  	<tr>
				...
				if($chart_type=="days")
				{
					...
					$tableStat = DataTable("user_statistics","WHERE user='$AuthUserName' ORDER BY id DESC");	
					...
					while($arrStat = mysql_fetch_array($tableStat))
					{
						...
						if(array_key_exists($arrInfo[0], $arrStatValues))
						{
							...
							if(isset($check_all))
							{
								...
								$strRef = "";
								$strRef2 = $BLOG_DOMAIN . "/blog.php?user=".$AuthUserName;
								
								if($BLOG_URL_FORMAT == 1)
								{
									$strRef = $AuthUserName .".". $BLOG_DOMAIN;
								}
								else
								{
									$strRef = $BLOG_DOMAIN ."/".$AuthUserName;
								}
							
								if(strstr($arrStat["referer"], $strRef2))
								{
									$arrStatValues2[$arrInfo[0]] ++;
								}
								else
								if(strstr($arrStat["referer"], $strRef))
								{
									$arrStatValues2[$arrInfo[0]] ++;
								}
								else
						...
						for($i=0;$i<sizeof($arrStatValues);$i++)
						{
							...
							if(isset($check_all))
							{
								echo "</td><td class=basictext width=90><b><font color=#00039c>[".$strCurrentKey."]</font><br></b><i>$UNIQUES:</i> <b><font color=#ff9a00>".$arrStatValues[$strCurrentKey]."</font></b><br> <i>$RELOADS:</i> <b><font color=#ff9a00>".$arrStatValues2[$strCurrentKey]."</font></b></td></tr></table><br>";		
							}
							else
							{
								echo "</td><td class=basictext width=90><b><font color=#00039c>[".$strCurrentKey."]</font></b><br><i>$TOTAL:</i><b><font color=#ff9a00>".$arrStatValues[$strCurrentKey]."</font></b></td></tr></table><br>";		
							}
						}		
				}
				?>
				...
				if($chart_type=="months")
				{
					...
					$tableStat = DataTable("user_statistics","WHERE user='$AuthUserName' ");	
...
(
	"user_statistics",
	array("date_special","referer"),
	array($DATE_MESSAGE,$REFERER),
	"100%",
	"WHERE user='".$AuthUserName."' AND trim(referer) <> ''  AND referer NOT LIKE '%".$BLOG_DOMAIN."%' ORDER BY id DESC",
	"",
	"",
...
<script>
var HTType="1";
var HTMessage="<?php echo $T_STAT_VISITS;?>";
